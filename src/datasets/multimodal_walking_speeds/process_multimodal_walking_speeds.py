...
def _prepare_static_tmp_dir(subject_dir: Path) -> tuple[tempfile.TemporaryDirectory, int]:
    ...
    for st in st_files:
        # Convert "2014011_ST.c3d" -> "2014011_ST_00.c3d"
...
def main() -> None:
    ...
    parser.add_argument(
        "--raw-root",
        default="data/multimodal_walking_speeds/raw",
        help="Root with subject folders and C3D files.",
    )
    parser.add_argument(
        "--eurobench-root",
    ...
    parser.add_argument(
        "--subject",
        default=None,
        help="Process one subject (e.g., 2014011).",
    )
    parser.add_argument(
        "--no-static",
        action="store_true",
    ...
    for subject_dir in subject_dirs:
        rows.append(
            convert_subject(
                ...
                save_analogs=not args.no_analogs,
            )
        )
